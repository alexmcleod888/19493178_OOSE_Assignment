...
/*Author: Alex McLeod
 *Purpose: abstract class inherited by FacebookMessengerSub that simulates the actions of a facebook messenger. Allowing messages to be sent to people and to detect
 *         what keywords are trending on facebook.
...
public abstract class FacebookMessenger 
{
	...
	protected Map<String, Integer> trendingKeywords;//Map of keywords whos occurrence on facebook are being tracked in the current search cycle
    ...
    public FacebookMessenger(long startTime)
    {
    ...
    //default contructor for when we are creating a facebook messenger just for sending messages and not searching for keywords
    ...
    //purpose: method for setting the current keywords to search for on facebook
    public void setKeywords(Set<String> newKeywords)
    {
    	...
    	for(String keyword : newKeywords)//for each new keyword 
    	{
    	...
    	//remove keywords that have been removed from policies during the last cycle
    ...
    /*STUB*/
    //Purpose: template method that simulates facebook messenger searching throughout facebook for each keyword 
    ...
    public Map<String, Integer> stubKeywordTest()
    {
    	...
    	return trendingKeywords;//return trendingKeywords which has had the occurence values for each keyword added to by keywordsDetected
    }
